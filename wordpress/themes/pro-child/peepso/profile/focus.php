    ...
    <div class="ps-focus__info">
      <div class="ps-focus__title">
        ...
        <div class="ps-focus__name" data-hover-card="<?php echo $PeepSoUser->get_id() ?>">
          ...
          ?>
          <span style="font-size: 11px; color: #444; font-weight: 500; margin-left: 5px;">
            <?php
            $bgc_profile = get_user_meta($PeepSoUser->get_id(), '_bgc_profile', true); 
            if(!empty($bgc_profile)){ echo '<span>'.$bgc_profile.' </span>'; }
            $bgc_type = get_user_meta($PeepSoUser->get_id(), '_bgc_type', true);
            if(!empty($bgc_type)){ echo '<span>'.$bgc_type.'</span>'; }
            ?>
          </span>
        </div>
        <?php
...
$peepso_user_id = $PeepSoUser->get_id();
echo do_shortcode('[cs_component id="lVrjaNSFNfERGE2kM1" parameters=\'{ "payto": "' . $peepso_user_id . '", "payfrom": "' . $current_user_id . '" }\']');
echo do_shortcode('[cs_component id="yB7Qqki9MfRu0b0SYe" parameters=\'{ "payto": "' . $peepso_user_id . '", "payfrom": "' . $current_user_id . '" }\']');
echo do_shortcode('[cs_component id="W9kPjDKY7ACiIgrul5" parameters=\'{ "payto": "' . $peepso_user_id . '", "payfrom": "' . $current_user_id . '" }\']');
