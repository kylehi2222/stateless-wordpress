...
// Existing code for processing wpaicg_chat_addition_text
...
// Handling bgc_data_flat
if (strpos($wpaicg_chat_addition_text, '[bgc_data_flat]') !== false) {
    $bgc_data_flat = get_bgc_data_flat();
    $wpaicg_chat_addition_text = str_replace('[bgc_data_flat]', $bgc_data_flat, $wpaicg_chat_addition_text);
}

// Handling bgc_data_post_flat
if (strpos($wpaicg_chat_addition_text, '[bgc_data_post_flat]') !== false) {
    if (isset($_REQUEST['post_id']) && !empty($_REQUEST['post_id'])) {
        $post_id = sanitize_text_field($_REQUEST['post_id']);
        $bgc_data_post_flat = get_bgc_data_post_flat($post_id);

        if ($bgc_data_post_flat) {
            $wpaicg_chat_addition_text = str_replace('[bgc_data_post_flat]', $bgc_data_post_flat, $wpaicg_chat_addition_text);
        } else {
            $wpaicg_chat_addition_text = str_replace('[bgc_data_post_flat]', 'No data available', $wpaicg_chat_addition_text);
        }
    } else {
        $wpaicg_chat_addition_text = str_replace('[bgc_data_post_flat]', 'No post_id provided', $wpaicg_chat_addition_text);
    }
}

// Handling post_title
if (strpos($wpaicg_chat_addition_text, '[post_title]') !== false) {
    if (isset($_REQUEST['post_id']) && !empty($_REQUEST['post_id'])) {
        $post_id = sanitize_text_field($_REQUEST['post_id']);
        $post_title = get_sanitized_post_title($post_id);

        if ($post_title) {
            $wpaicg_chat_addition_text = str_replace('[post_title]', $post_title, $wpaicg_chat_addition_text);
        } else {
            $wpaicg_chat_addition_text = str_replace('[post_title]', 'No title available', $wpaicg_chat_addition_text);
        }
    } else {
        $wpaicg_chat_addition_text = str_replace('[post_title]', 'No post_id provided', $wpaicg_chat_addition_text);
    }
}

// Handling post_content with sanitization
if (strpos($wpaicg_chat_addition_text, '[post_content]') !== false) {
    if (isset($_REQUEST['post_id']) && !empty($_REQUEST['post_id'])) {
        $post_id = sanitize_text_field($_REQUEST['post_id']);
        $post_content = get_sanitized_post_content($post_id);

        if ($post_content) {
            $wpaicg_chat_addition_text = str_replace('[post_content]', $post_content, $wpaicg_chat_addition_text);
        } else {
            $wpaicg_chat_addition_text = str_replace('[post_content]', 'Context for ' . $post_title . ' life has not been provided.', $wpaicg_chat_addition_text);
        }
    } else {
        $wpaicg_chat_addition_text = str_replace('[post_content]', 'No post_id provided', $wpaicg_chat_addition_text);
    }
}
        ...
        public function getChatEndpointModels() {
            ...
            $chatModels = ['gpt-4', 'gpt-4-32k', 'gpt-4-1106-preview','gpt-4o', 'gpt-4o-mini','gpt-4-turbo','gpt-4-vision-preview', 'gpt-3.5-turbo', 'gpt-3.5-turbo-16k'];
