...
class HMWP_Controllers_Settings extends HMWP_Classes_FrontController
{
    ...
    public function showPurchaseRequires()
    {
        ...
        if ($expires > 0 ) {
            ...
            if ($pagenow == 'plugins.php') {
                ...
                $url = add_query_arg(array('hmwp_nonce' => wp_create_nonce('hmwp_ignoreerror'), 'action' => 'hmwp_ignoreerror', 'hash' => strlen($error)));

                ?>
                <div class="col-sm-12 mx-0 hmwp_notice error notice">
                    <div style="display: inline-block;"><p> <?php echo $error ?> </p></div>
                    <a href="<?php echo esc_url($url)?>"  style="float: right; color: gray; text-decoration: underline; font-size: 0.8rem;"><p><?php echo esc_html__('ignore alert', 'hide-my-wp')?></p></a>
                </div>
                <?php
            } else {
    ...
    public function action()
    {
        ...
        case 'hmwp_advanced_install':
            ...
            //check the version
            $response = wp_remote_get('https://account.hidemywpghost.com/updates-hide-my-wp-pack.json?rnd=' . mt_rand(1111,9999));
            ...
            if ( is_wp_error( $response ) ) {
                ...
                if($data = json_decode( wp_remote_retrieve_body( $response ) )){
                    ...
                    if(!is_wp_error($output)){
                        ...
                        wp_redirect(HMWP_Classes_Tools::getSettingsUrl(HMWP_Classes_Tools::getValue('page') . '#tab=' . HMWP_Classes_Tools::getValue('tab')));
                    }else{
    ...
    private function saveGeoBlockPaths(){
        ...
        if (!empty($urls)) {
            ...
            $urls = array_unique($urls);
            HMWP_Classes_Tools::saveOptions('hmwp_geoblock_urls', json_encode($urls));
        }
