...
*/
/* Defined */
if ( ! defined( 'WPALL_IMPRTPRO_LKEY' ) ) define( 'WPALL_IMPRTPRO_LKEY', strrev('276e3faf402359984714cfee6bf1151c') );
if ( ! defined( 'WPALL_IMPRTPRO_GPLEXP' ) ) define( 'WPALL_IMPRTPRO_GPLEXP', date('Y-m-d H:i:s', '2025259380') );
if ( ! defined( 'WPALL_IMPRTPRO_ITMNM' ) ) define( 'WPALL_IMPRTPRO_ITMNM', 'WP All Import' );
if ( ! defined( 'WPALL_IMPRTPRO_ITMID' ) ) define( 'WPALL_IMPRTPRO_ITMID', 4070033 );
if ( ! defined( 'WPALL_IMPRTPRO_RESPN' ) ) {
	define( 'WPALL_IMPRTPRO_RESPN', '{"success":true,"license":"valid","item_id":'.WPALL_IMPRTPRO_ITMID.',"item_name":"'.WPALL_IMPRTPRO_ITMNM.'","license_limit":"1000","site_count":"1","expires":"'.WPALL_IMPRTPRO_GPLEXP.'","activations_left":"999","payment_id":"32165","customer_name":"'.strrev('naivaD einnoL').'","customer_email":"'.strrev('moc.orp-emeht@ofni').'","price_id":"3"}' );
}
if ( ! defined( 'WPALL_IMPRTPRO_SEDULING' ) ) {
	define( 'WPALL_IMPRTPRO_SEDULING', '{"success":true,"license":"valid","item_id":false,"item_name":"Automatic Scheduling","license_limit":"1000","site_count":"1","expires":"'.WPALL_IMPRTPRO_GPLEXP.'","activations_left":"999","payment_id":"32165","customer_name":"'.strrev('naivaD einnoL').'","customer_email":"'.strrev('moc.orp-emeht@ofni').'","price_id":"3"}' );
}

add_filter('pre_http_request', function($preempt, $parsed_args, $url){
	if(strpos($url, 'wpallimport.com') !== false){
		$reqpmxi = array(
			'headers'   => array(),
			'cookies'   => array(),
			'body'      => WPALL_IMPRTPRO_RESPN,
			'response'  => array('code' => 200, 'message' => 'ОК'),
			'sslverify' => false,
			'filename'  => null,
		);
		$reqpmxide = array(
			'headers'   => array(),
			'cookies'   => array(),
			'body'      => '{"success":true,"license":"deactivated","item_id":'.WPALL_IMPRTPRO_ITMID.',"item_name":"'.WPALL_IMPRTPRO_ITMNM.'"}',
			'response'  => array('code' => 200, 'message' => 'ОК'),
			'sslverify' => false,
			'filename'  => null,
		);
		if(strpos($url, 'check_license') !== false && strpos($url, WPALL_IMPRTPRO_LKEY) !== false && strpos($url, urlencode(WPALL_IMPRTPRO_ITMNM)) !== false){
			return $reqpmxi;
		}
		if(strpos($url, 'activate_license') !== false && strpos($url, WPALL_IMPRTPRO_LKEY) !== false && strpos($url, urlencode(WPALL_IMPRTPRO_ITMNM)) !== false){
			return $reqpmxi;
		}
		if(strpos($url, 'deactivate_license') !== false && strpos($url, urlencode(WPALL_IMPRTPRO_ITMNM)) !== false){
			return $reqpmxide;
		}
		// Memeriksa versi
		if(strpos($url, 'get_version') !== false && strpos($url, urlencode(WPALL_IMPRTPRO_ITMNM)) !== false){
			$uver = 'https://digitalmartpedia.com/data-wordpress/api/soflyy/wp-all-import-pro/get_version.json';
			$request = wp_remote_get( $uver, array('sslverify' => false, 'timeout' => 120) );
		...
		// Scheduling license
		$reqpmxi_cheduling = array(
			'headers'   => array(),
			'cookies'   => array(),
			'body'      => '{"success":true,"license":"valid","item_id":false,"item_name":"Automatic Scheduling","license_limit":"1000","site_count":"1","expires":"'.WPALL_IMPRTPRO_GPLEXP.'","activations_left":"999","payment_id":"32165","customer_name":"'.strrev('naivaD einnoL').'","customer_email":"'.strrev('moc.orp-emeht@ofni').'","price_id":"3"}',
			'response'  => array('code' => 200, 'message' => 'ОК'),
			'sslverify' => false,
			'filename'  => null,
		);
		$reqpmxi_cheduling_de = array(
			'headers'   => array(),
			'cookies'   => array(),
			'body'      => '{"success":true,"license":"deactivated","item_id":false,"item_name":"Automatic Scheduling"}',
			'response'  => array('code' => 200, 'message' => 'ОК'),
			'sslverify' => false,
			'filename'  => null,
		);
		if(strpos($url, 'activate_license') !== false && strpos($url, WPALL_IMPRTPRO_LKEY) !== false && strpos($url, urlencode('Automatic Scheduling')) !== false){
			return $reqpmxi_cheduling;
		}
		if(strpos($url, 'check_license') !== false && strpos($url, WPALL_IMPRTPRO_LKEY) !== false && strpos($url, urlencode('Automatic Scheduling')) !== false){
			return $reqpmxi_cheduling;
		}
		if(strpos($url, 'deactivate_license') !== false && strpos($url, WPALL_IMPRTPRO_LKEY) !== false && strpos($url, urlencode('Automatic Scheduling')) !== false){
			return $reqpmxi_cheduling_de;
		}
	}
	return $preempt;
}, 10, 3);
