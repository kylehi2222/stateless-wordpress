...
class DraftSubmissionsManager
{
    ...
    private function sendPartialEntryEmail($formData, $entryId, $formId, $action)
    {
        ...
        foreach ($fields as $field) {
            if (Arr::get($field, 'element') == 'save_progress_button' && Arr::get($field, "settings.send_email_on_entry_{$action}")) {
                $emailAddress = Arr::get($field, "settings.on_{$action}_email_address") ?: get_option('admin_email');
