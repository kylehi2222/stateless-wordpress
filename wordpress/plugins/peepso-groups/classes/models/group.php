<?php /*NWJjbDNsYng1QmhMczU4UHdsd3hjSzN1MjJBd0NCS1BHTURkV2JMbVFnWWRNU3hBeVJLKytTcXEzdXVOamJqS1FKK1oxRmpSUUEwK2U0d0J2UmpTQlBPSldWdUF3RHNQemhLZFlocjJHUlVxeWNyMkVEMUlZcE5lOTZDQS9Nd1FjS0RUdG5lQ0VjTW5ManFMWUVmTUdOajJwZVZOUFhYT1dTbVN1cUNoVjFvPQ==*/
...
class PeepSoGroup
{
    ...
    public static function validate($group_data)
    {
        ...
        foreach($group_data as $property=>$value) {
            ...
            if ( isset($rule['minlength']) && is_numeric($rule['minlength'])  && strlen(PeepSo3_Utilities_String::text_to_html_entities($value, ' ')) < $rule['minlength']) {
                $errors[$property] = sprintf( __('Minimum length is %s characters', 'groupso'), $rule['minlength'] );
            }

            if ( isset($rule['maxlength']) && is_numeric($rule['maxlength'])  && strlen(PeepSo3_Utilities_String::text_to_html_entities($value, ' ')) > $rule['maxlength'] ) {
    ...
    public function move_cover_file($src_file)
    {
        ...
        $resize = PeepSo::get_option('cover_width',3000);
        $compress = PeepSo::get_option('cover_quality', 85);

        $do_resize = $resize > 0;
        $do_compress = $compress < 100;

        $compress = min(100,$compress);

        if($do_resize || $do_compress) {
            $si = new PeepSoSimpleImage();
            $si->convert_image($src_file);
            $si->load($src_file);

            if($do_resize) {
                $si->resizeToWidth($resize);
            }

            $si->save($dest_file, IMAGETYPE_JPEG);

            $image = wp_get_image_editor($dest_file);
            $image->set_quality($compress);
