...
class Migrate_Integrations_Items {
	...
	public function perform_magic_button_migration() {
		...
		if ( defined( 'DOING_CRON' ) && 'yes' === get_option( 'automator_health_check_magic_button_moved', 'no' ) ) {
			return;
		}
  ...
  AND meta_key = %s",
		...
		if ( empty( $current_actions ) ) {
			...
			if ( defined( 'DOING_CRON' ) ) {
				update_option( 'automator_health_check_magic_button_moved', 'yes' );
			}
	...
	public function perform_run_code_migration() {
		...
		if ( defined( 'DOING_CRON' ) && 'yes' === get_option( 'automator_health_check_run_code_moved', 'no' ) ) {
			return;
		}
...
WHERE meta_value IN ('UOA_CALL_FUNC_CODE', 'UOA_CALL_FUNC_EVERYONE_CODE', 'UOADOACTION', 'UOADOACTION_EVERYONE', 'UOA_RUN_JS')
  AND meta_key = %s",
		...
		if ( empty( $current_actions ) ) {
			...
			if ( defined( 'DOING_CRON' ) ) {
				update_option( 'automator_health_check_run_code_moved', 'yes' );
			}
	...
	public function perform_generator_migration() {
		...
		if ( defined( 'DOING_CRON' ) && 'yes' === get_option( 'automator_health_check_generator_moved', 'no' ) ) {
			return;
		}
...
WHERE meta_value = %s
  AND meta_key = %s",
		...
		if ( empty( $current_actions ) ) {
			...
			if ( defined( 'DOING_CRON' ) ) {
				update_option( 'automator_health_check_generator_moved', 'yes' );
			}
	...
	public function update_webhooks_sample_values() {
		...
		if ( 'yes' === get_option( 'automator_health_check_webhook_sample_values_moved', 'no' ) ) {
			return;
		}
		...
		if ( empty( $current_actions ) ) {
			...
			update_option( 'automator_health_check_webhook_sample_values_moved', 'yes' );
