...
class PeepSoEmailDigest
{
	...
	function filter_build_email_digest_content($args)
	{
        ...
        if(stristr($post->post_content, PeepSo::BLOGPOSTS_SHORTCODE) && $blogpost = json_decode($post->post_content)) {
			...
			$embed_post = json_decode($post->post_content);
			if (isset($embed_post->post_id)) {
				$embed_post = get_post($embed_post->post_id);
				if (!empty($embed_post)) {
					$data['post_content'] = $embed_post->post_title;
				}
			}
		}
	...
	function filter_email_digest($post)
	{
		...
		if (class_exists('PeepSoSharePhotos')) {
			...
			$photos = $peepso_photos->get_photos_model()->get_post_photos($post->ID);
			$count = count($photos);
