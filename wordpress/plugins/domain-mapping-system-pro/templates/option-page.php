    ...
    <div class="dms-n">
        <form id="dms-map" method="post" action="<?= admin_url( 'admin-post.php' ); ?>">
            <div class="dms-n-row dms-n-config dms-n-config-fixed">
                ...
                <h3 class="dms-n-row-header"><?php _e( 'Domain Mapping System Configuration', 'domain-mapping-system' ) ?></h3>
                <p class="dms-n-row-subheader">
                    <span class="dms-n-row-subheader-important"><?php _e( 'Important!', 'domain-mapping-system' ) ?></span>
                    <span>
                         <?php
                         printf( __( 'This plugin requires configuration with your DNS host and on your server (cPanel, etc). Please see %1$sour documentation%2$s for configuration requirements.',
	                         'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' );
                         ?>
                </span>
                </p>
                <div class="dms-n-config-in">
                    <div id="domains" class="dms-n-config-container dms-tab-container">
						...
						<?php endif; ?>
                        <h3 class="dms-n-row-header dms-n-config-header"><?php _e( 'Domains', 'domain-mapping-system' ) ?></h3>
                        <div class="dms-n-row-footer">
                            <div class="dms-n-row-add">
                                ...
                                <a class="dms-add-row" href="#">
									<?php _e( '+ Add Domain Map Entry', 'domain-mapping-system' ) ?>
                                </a>
                            </div>
                        ...
                        <div class="dms-n-row-submit-wrapper">
                            <div class="dms-n-row-submit">
                                <input type="submit" value="<?php _e('Save', 'domain-mapping-system') ?>" class="dms-submit">
                        ...
                        <ul class="dms-n-api">
                            <li class="dms-n-row-subheader">
                                <a class="dms-n-row-subheader-link"
                                   ...
                                   target="_blank"><?php echo __( 'See our documentation for more', 'domain-mapping-system' ) ?></a>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="dms-n-row dms-n-additional">
                <div class="dms-n-additional-accordion opened">
                    <div class="dms-n-additional-accordion-header">
                        <h3>
                            <span><?php _e( 'Additional Options', 'domain-mapping-system' ) ?></span>
                        </h3>
                        <i></i>
                    </div>
                    <div class="dms-n-additional-accordion-body">
                        <ul>
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Global Domain Mapping', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'Enable Global Domain Mapping, which means that all pages will be served for all mapped domains. Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
                                        <span class="dms-main-domain-container">
                                            ...
                                            ?>
					                        <?php if ( ! empty( $mappings ) && count( $mappings ) > 1 ) { ?>
						                        <?php _e( 'Select the domain [+path] to serve for all unmapped pages:', 'domain-mapping-system' ) ?>
                                                <select name="dms_main_mapping"
                                                        class="dms-main-domain" <?= ! $dms_fs->can_use_premium_code__premium_only() ? 'disabled=disabled' : '' ?>>
                                                        <option value="0"><?= __( 'Select domain', 'domain-mapping-system' ) ?></option>
                                                        <?php
                                                        foreach ( $mappings as $domain ) {
	                                                        if ( ! empty( $domain->host ) && ! empty( $domain->path ) && ! empty( $domain->values ) )
		                                                        ?>
                                                                <option value="<?= $domain->id ?>" <?= $main_mapping == $domain->id ? 'selected' : '' ?> ><?= $domain->host . (! empty( $domain->path ) ? '/' . $domain->path : '') ?></option>
                                                        <?php }
                                                        ?>
                                                    </select>
					                        <?php }
				                        ...
				                        if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                            <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
						                        <?php _e( 'Upgrade', 'domain-mapping-system' ) ?>&#8594;
                                            </a>
				                        <?php endif; ?>
                                    </div>
                                </div>
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Global Archive Mapping', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'All posts within an archive or category automatically map to the specified domain (archive mappings override Global Domain Mapping). Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
				                        <?php
				                        if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                            <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Global Parent Page Mapping', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'Automatically map all pages attached to a Parent Page.  Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
				                        <?php
				                        if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                            <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Global Product Mapping', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'When you map a domain to the Shop page, all products on your site will be available through that domain. Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
				                        <?php
				                        if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                            <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Redirection', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'Force site visitors to see only mapped domains of a page (e.g. - disallow visitors to see the primary site domain version of a page). Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                             <strong><?php _e( 'URL Rewriting', 'domain-mapping-system' ) ?></strong> -
                                            <?php  _e( 'Rewrite all URLs on a mapped domain with:', 'domain-mapping-system' ) ?>
	                                        <?php $rewrite_scenario = get_option( 'dms_rewrite_urls_on_mapped_page_sc' ); ?>
                                            <select name="dms_rewrite_urls_on_mapped_page_sc"
                                                ...
                                                <?php echo sprintf( __( '%s Warning: %s  Global Rewriting may create dead links if you haven’t mapped internally linked pages properly. Read more in our %s Documentation > %s',
	                                                'domain-mapping-system' ), '<strong>', '</strong>', '<a class="info" href="https://docs.domainmappingsystem.com/features/url-rewriting" target="_blank" >', '</a>' ); ?>
	                                        <?php
	                                        if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                                <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
                                                    <?php _e( 'Upgrade', 'domain-mapping-system' ) ?>&#8594;
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Parent Page Slugs', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'Remove Parent Page slugs from mapped Child Page URLs. Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
				                        <?php
				                        if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                            <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    <strong><?php _e( 'Yoast SEO', 'domain-mapping-system' ); ?></strong>
                                </div>
                            </li>
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                                <strong><?php _e( 'Duplicate Yoast SEO Options', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'Each mapped page will have duplicated Yoast SEO options for each mapped domain tied to it.  Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
										<?php
										if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                            <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Yoast Sitemap per Domain', 'domain-mapping-system' ) ?></strong> -
                                            <?php printf( __( 'Dynamically generate a unique sitemap per domain. Read more in our %1$s Documentation%2$s.', 'domain-mapping-system' ), '<a class="dms-n-row-subheader-link" target="_blank" href="https://docs.domainmappingsystem.com">', '</a>' ) ?>
                                        </span>
										<?php
										if ( ! $dms_fs->can_use_premium_code__premium_only() ): ?>
                                            <a class="upgrade" href="<?= $dms_fs->get_upgrade_url(); ?>">
                            ...
                            <li>
                                <div class="dms-n-additional-accordion-li del">
                                    ...
                                    <div class="dms-n-additional-accordion-content">
                                        <span class="label">
                                            <strong><?php _e( 'Data Removal', 'domain-mapping-system' ) ?></strong> -
                                            <?php  _e( 'Delete plugin, data, and settings (full removal) when uninstalling.', 'domain-mapping-system' ) ?>
                                            <?php echo sprintf( __( '%s Warning: %s This action is irreversible.', 'domain-mapping-system' ), '<strong>', '</strong>' ); ?>
                                        </span>
                                    </div>
                                </div>
                            </li>
                        ...
                        <div class="dms-n-row-submit-wrapper">
                            <div class="dms-n-row-submit">
                                <input type="submit"
                                       value="<?php _e( 'Save', 'domain-mapping-system' ) ?>"
                                       class="dms-submit"/>
                                <div class="dms-n-loader"></div>
                            </div>
                        </div>
            ...
            <div class="dms-n-row dms-n-post-types">
                <h3 class="dms-n-row-header"><?php _e( 'Available Post Types', 'domain-mapping-system' ); ?></h3>
                <p class="dms-n-row-subheader"><?php _e( 'Select the Post Types or Custom Taxonomies that should be available for Domain Mapping System.', 'domain-mapping-system' ); ?></p>
                <div class="dms-n-post-types-in">
                    <div class="dms-n-post-types-container">
                        <label class="dms-n-post-types-label <?= get_option( 'dms_use_page' ) == 'on' ? 'checked' : ''; ?>"
                            ...
                            <span>
                                <?php _e( 'Pages', 'domain-mapping-system' ); ?>
                            </span>
                        </label>
						<?php if ( $dms_fs->can_use_premium_code__premium_only() ): ?>
							<?php $page_taxonomies = \DMS\Includes\Utils\Helper::get_custom_taxonomies( 'page' );
                        ...
                        <label class="dms-n-post-types-label <?= get_option( 'dms_use_post' ) == 'on' ? 'checked' : ''; ?>"
                            ...
                            <span>
                                <?php _e( 'Posts', 'domain-mapping-system' ); ?>
                            </span>
                        </label>
						<?php if ( $dms_fs->can_use_premium_code__premium_only() ): ?>
							<?php $post_taxonomies = \DMS\Includes\Utils\Helper::get_custom_taxonomies( 'post' );
                        ...
                        <label class="dms-n-post-types-label <?= get_option( 'dms_use_categories' ) == 'on' ? 'checked' : ''; ?>"
                            ...
                            <span>
                        <?php _e( 'Blog Categories', 'domain-mapping-system' ); ?>
                    </span>
                        </label>
						<?php
						...
						foreach ( $types as $type ) {
							...
							?>
                            <label class="dms-n-post-types-label <?= ( $value == "on" ) ? ' checked' : '' ?>"
                                   for="dms_use_<?= $type['name'] ?>">
                                <input class="dms-n-post-types-checkbox" name="dms_use_<?= $type['name'] ?>"
                                       type="checkbox" id="dms_use_<?= $type['name'] ?>"
									<?= $value == 'on' ? 'checked="checked"' : '' ?>>
							...
							if ( ! empty( $type['has_archive'] ) ) {
								...
								?>
                                <label class="dms-n-post-types-label <?= ( $value == "on" ) ? ' checked' : '' ?>"
                                       for="dms_use_<?= $type['name'] ?>_archive">
                                    <input class="dms-n-post-types-checkbox" name="dms_use_<?= $type['name'] ?>_archive"
                                           type="checkbox"
                                           id="dms_use_<?= $type['name'] ?>_archive" <?= $value == 'on' ? 'checked="checked"' : '' ?>>
                                    <span><?= $type['label'] ?><strong><?= __( 'Archive', 'domain-mapping-system' ) ?></strong></span>
                                </label>
								<?php
							}
							if ( $dms_fs->can_use_premium_code__premium_only() && ! empty( $type['taxonomies'] ) ) : ?>
								<?php foreach ( $type['taxonomies'] as $taxonomy ): ?>
                                    ...
                                    <label class="dms-n-post-types-label <?= ( $value == "on" ) ? ' checked' : '' ?>"
                                           for="dms_use_cat_<?= $type['name'] ?>_<?= $taxonomy['name'] ?>">
                                        <input class="dms-n-post-types-checkbox"
                                               name="dms_use_cat_<?= $type['name'] ?>_<?= $taxonomy['name'] ?>"
                                               type="checkbox"
                                               id="dms_use_cat_<?= $type['name'] ?>_<?= $taxonomy['name'] ?>"
											<?= $value == 'on' ? 'checked="checked"' : '' ?>>
