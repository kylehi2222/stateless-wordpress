...
class Mapping_Scenario {
	...
	/**
	 * Loop through Force redirections scenarios
	 *
	 * @param Force_Redirection_Handler $handler Force redirection handler instance
	 * @param Request_Params $request_params Request params handler instance
	 *
	 * @return null|string
	 */
	public function run_force_redirection_scenario__premium_only( Force_Redirection_Handler $handler, Request_Params $request_params ):?string {
		...
		foreach ( $list as $scenario ) {
			if (class_exists($scenario)) {
				...
				if ( $uri = $scenario_instance->force_redirection__premium_only( $handler, $request_params ) ) {
					break;
				}
			}
		}
