...
class Mapping_Handler {
	...
	public function define_hooks(): void {
		...
		add_filter( 'wp_redirect', array( $this, 'prevent_redirection' ), 9999, 2 );
	}
		...
		try {
			if ( $query->is_main_query() && ! is_admin() ) {
				...
				if ( $this->domain_path_match ) {
					$this->mapping_values = $this->mapping ? Mapping_Value::where( [ 'mapping_id' => $this->mapping->id ] ) : [];
	...
	public function matching_mapping_from_db(): ?Mapping {
		try {
			...
			$args         = [ 'host' => $this->request_params->get_domain() ];
	...
	/**
	 * Define favicon and custom html in the head
	 *
	 * @return void
	 */
	public function add_favicon_and_custom_html__premium_only(): void {
		if ( class_exists( '\\DMS\\Includes\\Frontend\\Handlers\\Favicon_Handler' ) ) {
			add_filter( 'get_site_icon_url', array(
				new Favicon_Handler( $this->mapping->attachment_id ),
				'override'
			), 10, 1 );
		}
		if ( class_exists( '\\DMS\\Includes\\Frontend\\Handlers\\Map_Html_Handler' ) ) {
			add_action( 'wp_head', array(
				new Map_Html_Handler( $this->mapping->custom_html ),
				'override'
			) );
		}
	}
	...
	/**
	 * Call global domain mapping functionality
	 *
	 * @return void
	 */
	public function check_global_domain_existence__premium_only(): void {
		add_action( 'wp', array( $this->global_domain_mapping, 'init' ), 11 );
	}
