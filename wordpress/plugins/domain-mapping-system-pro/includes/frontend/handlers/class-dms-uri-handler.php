...
class URI_Handler {
	...
	public function prepare_assets_uri_filters(): void {
		add_filter( 'plugins_url', array( $this, 'rewrite_plugins_url' ), 99, 3 );
		add_filter( 'rest_url', array( $this, 'rewrite_rest_url' ), 99, 2 );
		add_filter( 'script_loader_src', array( $this, 'replace_script_style_src' ), 10, 2 );
		add_filter( 'style_loader_src', array( $this, 'replace_script_style_src' ), 10, 2 );
		add_filter( 'admin_url', array( $this, 'rewrite_admin_url' ), 999, 4 );
		add_filter( 'script_module_loader_src', array( $this, 'rewrite_script_modules_src' ), 10, 2 );
		add_filter( 'wp_get_attachment_image_src', array( $this, 'rewrite_attachment_src' ), 10, 4 );
		add_filter( 'get_header_image_tag', array( $this, 'rewrite_header_image_markup' ), 10, 3 );
		add_filter( 'wp_calculate_image_srcset', array( $this, 'rewrite_image_srcset' ), 10, 5 );
		add_filter( 'template_directory_uri', array( $this, 'rewrite_template_uri' ), 10, 3 );
		add_filter( 'stylesheet_directory_uri', array( $this, 'rewrite_stylesheet_uri' ), 10, 3 );
		add_filter( 'the_content', array( $this, 'rewrite_the_content' ), 10, 1 );
		// Divi related
		add_filter( 'et_builder_custom_fonts', array( $this, 'rewrite_et_builder_custom_fonts' ) );
		add_filter( 'et_core_page_resource_tag', array( $this, 'rewrite_et_core_page_resource_tag' ), 999, 5 );
	}

	/**
	 * Prepare premium__only uri filters
	 *
	 * @return void
	 */
	public function prepare_uri_filters__premium_only(): void {
		if ( ! $this->is_allowed_to_rewrite_links() ) {
			return;
		}
		add_filter( 'home_url', array( $this, 'rewrite_home_url__premium_only' ), 99, 4 );
		add_filter( 'paginate_links', array( $this, 'rewrite_pagination_links__premium_only' ), 99 );
		add_filter( 'nav_menu_link_attributes', array( $this, 'rewrite_nav_menu_link__premium_only' ), 99, 4 );
		add_filter( 'page_link', array( $this, 'rewrite_page_link__premium_only' ), 99, 4 );
		add_filter( 'post_link', array( $this, 'rewrite_post_link__premium_only' ), 99, 4 );
		add_filter( 'post_type_link', array( $this, 'rewrite_post_type_link__premium_only' ), 99, 4 );
		add_filter( 'term_link', array( $this, 'rewrite_term_link__premium_only' ), 1, 3 );
	}
	...
	public function replace_host_occurrence( $data ): string {
		...
		if ( $this->request_params->is_subdirectory_install() ) {
			...
			return preg_replace_callback( '/(https?:\/\/)(' . $host . ')((\/' . $path . '\/\w+)*\/)?([\w\-.]+[^#?\s]+)' . $dot . '(#[\w\-]+)?/', array(
				$this,
				'actual_host_replace'
			), $data, - 1 ) ?? $data;
		}

		return preg_replace_callback( '/(https?:\/\/)(' . $host . ')((\/\w+)*\/)?([\w\-.]+[^#?\s]+)' . $dot . '(#[\w\-]+)?/', array(
			$this,
			'actual_host_replace'
		), $data, - 1 ) ?? $data;
	}
	...
	/**
	 * Rewrite homepage url
	 *
	 * @param  string  $url
	 *
	 * @return string|array|string[]
	 */
	public function rewrite_home_url__premium_only( string $url ) {
		$home_link = get_option( 'home' );
		if ( rtrim( $home_link, '/' ) === rtrim( $url, '/' ) ) {
			$page_on_front = get_option( 'page_on_front' );
			if ( ! empty( $page_on_front ) ) {
				$new_link = $this->get_rewritten_url( $page_on_front, $url );
				if ( ! empty( $new_link ) ) {
					return $new_link;
				}
			}
	...
	public function get_rewritten_url( $key, ?string $link ): ?string {
		try {
			...
			if ( $this->rewrite_scenario == self::REWRITING_SELECTIVE ) {
				$mapping        = null;
				$mapping_values = Mapping_Value::where( [ 'object_id' => $key ] );
	 ...
	 * @return string
	 */
	public function rewrite_pagination_links__premium_only( $link ): string {
		return self::replace_host_occurrence( $link );
	}

	/**
	 * Rewrite navigation menu link
	 *
	 * @param $attributes
	 * @param $item
	 *
	 * @return array
	 */
	public function rewrite_nav_menu_link__premium_only( $attributes, $item ): array {
		if ( $item->type == 'post_type' && ( $item->object == 'page' || $item->object == 'post' ) ) {
			...
			$href = $this->get_rewritten_url( null, $attributes['href'] );
			if ( ! empty( $href ) ) {
				$attributes['href'] = $href;
			}
		}
		if ( ! empty( $key ) ) {
			$href = $this->get_rewritten_url( $key, $attributes['href'] );
			if ( ! empty( $href ) ) {
				$attributes['href'] = $href;
			}
		}
	 ...
	 * @return string
	 */
	public function rewrite_page_link__premium_only( $link, $post_id ): string {
		$new_link = $this->get_rewritten_url( $post_id, $link );
		if ( ! empty( $new_link ) ) {
			return $new_link;
		}
	 ...
	 * @return string
	 */
	public function rewrite_post_link__premium_only( $permalink, $post ): string {
		$new_link = $this->get_rewritten_url( $post->ID, $permalink );
		if ( ! empty( $new_link ) ) {
			return $new_link;
		}
	...
	/**
	 * Rewrite custom post type links
	 *
	 * @param $permalink
	 * @param $post
	 *
	 * @return string
	 */
	public function rewrite_post_type_link__premium_only( $permalink, $post ): string {
		$new_link = $this->get_rewritten_url( $post->ID, $permalink );
		if ( ! empty( $new_link ) ) {
			return $new_link;
		}
	 ...
	 * @return string
	 */
	public function rewrite_term_link__premium_only( $termlink, $term, $taxonomy ): string {
		...
		$new_link = $this->get_rewritten_url( $key, $termlink );
		if ( ! empty( $new_link ) ) {
			return $new_link;
		}
