...
class WP_Queried_Object_Handler {
	 ...
	 */
	public function __construct( Frontend $frontend, ?Global_Domain_Mapping_Handler $global_domain_mapping_handler, Mapping_Handler $mapping_handler, Request_Params $request_params ) {
			...
			if ( ! is_admin() && ( ! empty( $this->global_domain_mapping_handler->mapped ) || $this->mapping_handler->domain_path_match ) && ! empty( $this->request_params->path ) ) {
				...
				if ( $result instanceof Mapping ) {
					...
					add_filter( 'get_site_icon_url', [
						new Favicon_Handler( $this->matched_mapping->attachment_id ),
						'override'
					] );
					add_action( 'wp_head', array(
						new Map_Html_Handler( $this->matched_mapping->custom_html ),
						'override'
					) );
	...
	/**
	 * Check the case when the object id is empty
	 * Almost impossible case
	 *
	 * @return string|null
	 */
	public function empty_object_scenario__premium_only(): ?string {
		if ( empty( $this->object_id ) ) {
			if ( $this->frontend->global_domain_mapping ) {
				$main_mapping = Setting::find( 'dms_main_mapping' )->get_value();
				if ( $main_mapping ) {
					$main_mapping = Mapping::where( [ 'id' => $main_mapping ] );
					$main_mapping = ! empty( $main_mapping ) ? $main_mapping[0] : null;
					if ( ! empty( $main_mapping->host )
					     && empty( $this->mapping_handler->mapped )
					     && $this->request_params->domain != $main_mapping->host
					     && ! empty( $main_mapping->path )
					     && ! str_starts_with( $this->request_params->path, (string) $main_mapping->path ) ) {

						return Helper::generate_url( $main_mapping->host, $main_mapping->path );
					}
				}
			}
		}
	...
	/**
	 * Check if the page was opened wrong path redirect to right path
	 *
	 * @return string|null
	 */
	public function wrong_path_scenario__premium_only(): ?string {
		if (empty($this->mapping_handler->matching_mapping_value->mapping_id)){
			return null;
		}
		$primary_value = Mapping::get_primary_mapping_value( $this->mapping_handler->matching_mapping_value->mapping_id );
		if ( $this->object_id == $primary_value->object_id
		     && ! empty( $this->mapping_handler->mapping->path )
		     && ( str_starts_with( $this->request_params->path, (string) $this->mapping_handler->mapping->path ) )
		     && $this->request_params->path != $this->mapping_handler->mapping->path ) {

			return Helper::generate_url( $this->request_params->domain, $this->mapping_handler->mapping->path );
		}
	...
	/**
	 * Check mapped cpt scenario
	 *
	 * @return string|null
	 */
	public function mapped_cpt_scenario__premium_only(): ?string {
		if ( ! Helper::is_mapped_cpt( $this->object_id ) && empty( $this->mapping_handler->mapped ) &&
		     !str_starts_with( $this->request_params->get_domain(), $this->request_params->get_base_host() ) ) {
			if ( $this->frontend->global_domain_mapping ) {
				$main_mapping = Mapping::get_main();
				if ( ! empty( $main_mapping->host ) && $this->request_params->domain != $main_mapping->host ) {
					return Helper::generate_url( $main_mapping->host, $main_mapping->path );
				}
			}
		}
