...
class Frontend {
	 ...
	 */
	public function __construct( string $plugin_url, string $plugin_name, string $version, string $plugin_dir_path ) {
		$this->set_plugin_params( $plugin_url, $plugin_name, $version, $plugin_dir_path );
		$this->set_options();
	 ...
	 * @return void
	 */
	private function set_plugin_params( $plugin_url, $plugin_name, $version, $plugin_dir_path ):void {
	...
	private function define_whether_is_dms_hosted() {
		$mapping = Mapping::where(['host' => $this->request_params->get_domain()]);

		$this->is_dms_hosted = !empty($mapping[0]);
	}

	/**
	 * Define premium__only handlers
	 *
	 * @return void
	 */
	public function handlers_init__premium_only(): void {
		$this->force_redirection_handler = new \DMS\Includes\Frontend\Handlers\Force_Redirection_Handler( $this->request_params, $this );
		$this->global_mapping_handler    = new \DMS\Includes\Frontend\Handlers\Global_Domain_Mapping_Handler( $this->request_params, $this->force_redirection_handler, $this );
	}
	...
	public function handlers_init(): void {
		...
		$this->wp_queried_object_handler = new WP_Queried_Object_Handler( $this, $this->global_mapping_handler, $this->mapping_handler, $this->request_params );
	}
	 ...
	 * @return Frontend
	 */
	public static function get_instance( string $plugin_url = '', string $plugin_name = '', string $version = '', string $plugin_dir_path = '' ): Frontend {
		if ( is_null( self::$_instance ) ) {
			self::$_instance = new self( $plugin_url, $plugin_name, $version, $plugin_dir_path );
		}
	...
	public function need_handlers_and_mapping_scenarios() {
		return
			( ! is_admin()
			  ...
			  && ! str_contains( $this->request_params->path, 'cornerstone' )
			  && ! str_contains( $this->request_params->path, 'themeco' )
			  && ! str_contains( $this->request_params->path, 'wp-json' )
			  && ! str_contains( $this->request_params->path, 'store-manager' )
			  && $this->is_dms_hosted
			);
	}
}