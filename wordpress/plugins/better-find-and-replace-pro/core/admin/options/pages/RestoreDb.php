...
use RealTimeAutoFindReplacePro\admin\options\functions\AllRestoreDbData;
...
class RestoreDb {
	...
	public function generate_page( $args ) {
		...
		if ( isset( $_GET['s'] ) && ! empty( $_GET['s'] ) ) {
			...
			$args['well'] = "<p class='search-keyword'> " . __( 'Search results for', 'real-time-auto-find-and-replace' ) . " : '<b>" . Util::check_evil_script( $_GET['s'] ) . "</b>' </p> <a href='{$back_url}' class='button'><< " . __( 'Back to all', 'real-time-auto-find-and-replace' ) . ' </a> ';
		}
