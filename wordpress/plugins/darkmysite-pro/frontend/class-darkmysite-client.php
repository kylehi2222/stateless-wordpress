    ...
    class DarkMySiteClient
    {
        ...
        function darkmysite_client_enqueue()
        {
            if($this->darkmysite_is_dark_mode_allowed()){
                wp_enqueue_style('darkmysite-client-main', DARKMYSITE_PRO_CSS_DIR.'client_main.css', array(), DARKMYSITE_PRO_VERSION);
                wp_enqueue_script( 'darkmysite-client-main', DARKMYSITE_PRO_JS_DIR.'client_main.js', array(), DARKMYSITE_PRO_VERSION);
            }
        ...
        function darkmysite_is_dark_mode_allowed()
        {
            ...
            if($this->utils->isRestrictedByDisallowedPages($this->data_settings["disallowed_pages"])){
                return False;
            }

            /* Disable Except Allowed Page */
            if($this->utils->isRestrictedByAllowedPages($this->data_settings["allowed_pages"])){
                return False;
            }

            /* Disable on Disallowed Post */
            if($this->utils->isRestrictedByDisallowedPosts($this->data_settings["disallowed_posts"])){
                return False;
            }

            /* Disable Except Allowed Post */
            if($this->utils->isRestrictedByAllowedPosts($this->data_settings["allowed_posts"])){
                return False;
            }
