...
class PeepSoUser
{
    ...
    private function get_name_parts($is_admin = FALSE, $override = FALSE)
    {
        ...
        if(PeepSo::is_admin() && PeepSo::get_option('site_reporting_enable', TRUE) && strlen(get_user_meta($this->id, 'peepso_reported', TRUE))) {
            $name_parts[] = ' - '.__('Profile reported', 'peepso-core');
        }
