...
class UPT_Sync
{
    ...
    function run_hooks() {
        ...
        add_action( 'save_post',        [ $this, 'set_usermeta_rel' ], 5, 2 );
    }
    ...
    /**
     * When a upt_user item is added, save a reference (_upt_post_id) into wp_usermeta
     */
    function set_usermeta_rel( $post_id, $post ) {
        if ( 'upt_user' == $post->post_type ) {
            update_user_meta( $post->post_author, UPT()->meta_key, $post_id );
        }
    }
    ...
    function run_sync( $user_id = false ) {
        ...
        if ( false === $user_id ) {
            ...
            if ( is_multisite() ) {
                ...
                $where .= " AND u.ID IN (SELECT `user_id` FROM {$wpdb->usermeta} WHERE `meta_key` = '$meta_key')";
            }
        }
        // Sync a user
        ...
        $where = apply_filters( 'upt_sync_where', $where );

        // Resume indexing?
        $offset = isset( $_POST['upt_offset'] ) ? (int) $_POST['upt_offset'] : 0;
        $attempt = isset( $_POST['upt_retries'] ) ? (int) $_POST['upt_retries'] : 0;
            ...
            if ( 60 < ( time() - $touch ) ) {
                $post_data = [
                    ...
                    'body'      => [
                        ...
                        'upt_offset'    => $num_synced,
                        'upt_retries'   => $retries + 1,
                        'touch'     => $touch
                    ]
                ];
