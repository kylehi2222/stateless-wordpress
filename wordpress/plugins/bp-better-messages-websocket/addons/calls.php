...
if ( !class_exists( 'Better_Messages_Calls' ) ):
    ...
    class Better_Messages_Calls
    {
        ...
        {
            $debug = Better_Messages()->functions->get_message_meta( $message_id, 'debug' );
                ...
                if (is_array($debug) && count($debug) > 0) {
                    foreach ($debug as $item) {
                        if (isset($item[1])) {
                            ...
                            if (isset($entry['bytesReceived'])) {
                                $bytesReceived += intval($entry['bytesReceived']);
                            }

                            if (isset($entry['bytesSent'])) {
                                $bytesSent += intval($entry['bytesSent']);
                            }
                        }
                    }
                }
