    ...
    $.fn.wpAssetCleanUpPro = function() {
        return {
            cssJsManagerActions: function() {
                let cbSelectorMakeExceptionOnPage = '.wpacu_load_it_option_on_this_page.wpacu_load_exception';

                // Load it on all "post/page/custom post types" pages if specific taxonomies (e.g. Category, Tag) are set on those pages
                $(document).on('change', '.wpacu_load_it_post_type_via_tax_checkbox', function () {
                    let $mainThis = $(this);
                    ...
                    if ($(this).prop('checked')) {
                        ...
                        if ($termsDd.html() === '') {
                            ...
                            $parentLi.find('.wpacu_handle_manage_post_type_via_tax_input_wrap')
                ...
                // e.g. /category/[cat_here]/, /tag/[tag_here]/
                $(document).on('change', cbSelectorMakeExceptionOnPage + ',' + '.wpacu_load_it_option_tax_type', function () {
                    ...
                    if ($(this).prop('checked')) {
                        ...
                        // e.g. If "On this page" (load exception) is clicked
                        if ($(this).hasClass('wpacu_load_it_option_on_this_page')) {
                            // 'On all pages of "taxonomy" type' gets unchecked (if already checked)
                            // As it doesn't make any sense to have both checked
                            if ($('#wpacu_load_it_option_tax_type_' + asset_type + '_' + handle).is(':checked')) {
                                $('#wpacu_load_it_option_tax_type_' + asset_type + '_' + handle).prop('checked', false).trigger('change');
                            }
                        }

                        // e.g. If 'On all pages of "taxonomy" type' is clicked
                        else if ($(this).hasClass('wpacu_load_it_option_tax_type')) {
                            // "On this page" gets unchecked (if already checked)
                            // As it doesn't make any sense to have both checked
                            if ($('#wpacu_load_it_option_' + asset_type + '_' + handle).is(':checked')) {
                                $('#wpacu_load_it_option_' + asset_type + '_' + handle).prop('checked', false).trigger('change');
                            } else if ($('#wpacu_bulk_unload_taxonomy_' + asset_type + '_' + handle).is(':checked')) {
                                $('#wpacu_bulk_unload_taxonomy_' + asset_type + '_' + handle).prop('checked', false).trigger('change');
                ...
                // Load it on all author archive pages, e.g. /author/[any_author_here]/
                $(document).on('change', cbSelectorMakeExceptionOnPage + ',' + '.wpacu_load_it_option_author_type', function () {
                    let handle = $(this).attr('data-handle');
                    ...
                    if ($(this).prop('checked')) {
                        ...
                        // e.g. If "On this page" (load exception) is clicked
                        if ($(this).hasClass('wpacu_load_it_option_on_this_page')) {
                            // 'On all auhor archive pages' gets unchecked (if already checked)
                            // As it doesn't make any sense to have both checked
                            if ($('#wpacu_load_it_option_author_type_' + asset_type + '_' + handle).is(':checked')) {
                                $('#wpacu_load_it_option_author_type_' + asset_type + '_' + handle).prop('checked', false).trigger('change');
                            }
                        }

                        // e.g. If 'On all author archive pages' is clicked
                        else if ($(this).hasClass('wpacu_load_it_option_author_type')) {
                            // "On this page" gets unchecked (if already checked)
                            // As it doesn't make any sense to have both checked
                            if ($('#wpacu_load_it_option_' + asset_type + '_' + handle).is(':checked')) {
                                $('#wpacu_load_it_option_' + asset_type + '_' + handle).prop('checked', false).trigger('change');
                            } else if ($('#wpacu_bulk_unload_author_' + asset_type + '_' + handle).is(':checked')) {
                                $('#wpacu_bulk_unload_author_' + asset_type + '_' + handle).prop('checked', false).trigger('change');
                                $(this).prop('checked', false).trigger('change');
                            }
                        }
                ...
                $(document).on('click change', '.wpacu_load_it_option_regex', function () {
                    let $parentLi = $(this).parents('li');
            ...
            limitSubmittedFields: function () {
                let preloadTargetInput    = '[data-wpacu-input="preload"]',
                    ...
                    postTypeViaTaxUnload  = '.wpacu_unload_it_post_type_via_tax_checkbox',
                    postTypeViaTaxLoad    = '.wpacu_load_it_post_type_via_tax_checkbox',
                ...
                if (wpacuListToCheck.length > 0) {
                    $(wpacuListToCheck.join()).each(function () {
                        ...
                        if ($(this).hasClass('wpacu_unload_it_post_type_via_tax_checkbox')) {
                            let dataHandle = $(this).attr('data-handle');
                            let dataFor = $(this).attr('data-handle-for');

                            let $targetedDd = $('select.wpacu_unload_post_type_via_tax_dd[data-handle="' + dataHandle + '"][data-handle-for="' + dataFor + '"]');
                        ...
                        if ($(this).hasClass('wpacu_load_it_post_type_via_tax_checkbox')) {
                            let dataHandle = $(this).attr('data-handle');
                            let dataFor = $(this).attr('data-handle-for');

                            let $targetedDd = $('select.wpacu_load_post_type_via_tax_dd[data-handle="' + dataHandle + '"][data-handle-for="' + dataFor + '"]')
            ...
            pluginLoadManager: function () {
                ...
                $('#' + wpacu_object.plugin_prefix + '_plugins_manager_checkbox').on('click', function() {
                    ...
                    let wpacuSpinnerElTextId = '#wpacu-main-loading-spinner-text';

                    if ($(wpacuSpinnerElId).length > 0) {
                        $(wpacuSpinnerElId).removeClass('wpacu_hide');
                        $(wpacuSpinnerElTextId).html($('div[data-wpacu-updating-text="1"]').html());
                    }
            ...
            wpacuTriggerChosenForTaxDd: function() {
                setTimeout(function() {
                    let wpacuManageViaTaxDdTarget = '.wpacu_chosen_select.wpacu_post_type_via_tax_dd';

                    if ($(wpacuManageViaTaxDdTarget).length > 0 && typeof $.fn.chosen !== 'undefined') {
                        $(wpacuManageViaTaxDdTarget).each(function () {
                            if ( ! $(this).next().hasClass('chosen-container-active') ) {
            ...
            wpacuAjaxLoadAllSetTermsForPostType: function($termsDd, $parentLi) {
                ...
                if ($termsDd.hasClass('wpacu_load_post_type_via_tax_dd')) {
        ...
        return {
            actions: function () {
                $(document).on('click', '#wpacu-mark-license-valid-button', function() {
                    return confirm(wpacu_object.mark_license_valid_confirm_msg);
                });
    ...
    $.fn.wpAssetCleanUp = function() {
        ...
        return {
                ...
                $(document).on('change', '.wpacu_bulk_unload', function (event) {
                    ...
                    if ($(this).prop('checked')) {
                        ...
                        if ($(this).hasClass('wpacu_global_unload') ||
                            ...
                            || $(this).hasClass('wpacu_author_unload')
                            // [/wpacu_pro]
                        ) {
                            /*
            ...
            wpacuParseResultsForHarcodedAssets: function (contents) {
                ...
                // [wpacu_pro]
                if (typeof wpacu_object.current_post_type !== 'undefined' && wpacu_object.current_post_type) {
                    dataGetLoadedHardcodedAssets['page_type'] = 'singular';
                    dataGetLoadedHardcodedAssets['current_post_type'] = wpacu_object.current_post_type;
                }

                if (typeof wpacu_object.is_404_page !== 'undefined' && wpacu_object.is_404_page) {
                    dataGetLoadedHardcodedAssets['page_type'] = '404';
                }

                if (typeof wpacu_object.is_search_page !== 'undefined' && wpacu_object.is_search_page) {
                    dataGetLoadedHardcodedAssets['page_type'] = 'search';
                }

                if (typeof wpacu_object.is_date_page !== 'undefined' && wpacu_object.is_date_page) {
                    dataGetLoadedHardcodedAssets['page_type'] = 'date';
                }

                if (typeof wpacu_object.is_tax_page !== 'undefined' && wpacu_object.is_tax_page && wpacu_object.tax_name) {
                    dataGetLoadedHardcodedAssets['page_type'] = 'tax';
                    dataGetLoadedHardcodedAssets['tax_name'] = wpacu_object.tax_name;
                }

                if (typeof wpacu_object.is_author_page !== 'undefined' && wpacu_object.is_author_page && wpacu_object.author_nice_name) {
                    dataGetLoadedHardcodedAssets['page_type'] = 'author';
                    dataGetLoadedHardcodedAssets['author_nice_name'] = wpacu_object.author_nice_name;
                }

                if (typeof wpacu_object.is_archive_page !== 'undefined' && wpacu_object.is_archive_page && wpacu_object.archive_name) {
                    dataGetLoadedHardcodedAssets['page_type'] = 'archive';
                    dataGetLoadedHardcodedAssets['archive_name'] = wpacu_object.archive_name;
                }
                // [/wpacu_pro]
